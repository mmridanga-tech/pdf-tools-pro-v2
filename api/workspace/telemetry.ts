...
export default async function telemetryHandler(req: any, res: any) {
  if (req.method !== 'GET') {
  ...
  if (!user) return; // 401 response already sent
  ...
  try {
    ...
    // 1. Get live in-memory server metrics
    const metrics = telemetryStore.getSystemMetrics();

    // 2. Fetch today's persistent atomic quota usage from Firestore
    ...
    // 3. Fallback/aggregate with Firestore logs if in-memory buffer was recently reset
    ...
    // 4. Rate-limit and security events

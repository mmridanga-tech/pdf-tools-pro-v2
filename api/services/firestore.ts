...
export async function getOrCreateUserDoc(uid: string, email?: string): Promise<FirestoreUserData> {
  try {
    ...
    if (snap.exists) {
      ...
      // Optionally update last login timestamp asynchronously
    ...
    // Graceful fallback if Firestore is unavailable in dev/offline mode
    return {
      uid,
      email: email || '',
      role: 'user',
 ...
 * Only modifies provided subscription fields without overwriting unrelated user properties.
 */
export async function updateUserSubscription(
  ...
  try {
    ...
    // Also write to separate `subscriptions` collection for history audit
...
export async function checkAndRecordWebhookEvent(
  ...
  try {
    ...
    return true; // Fallback to process if Firestore fails
  }
}
 ...
 * Document path: `aiUsage/{uid}_{YYYY-MM-DD}`
 */
export async function checkAndIncrementDailyUsageTransaction(
  ...
  try {
    ...
    // Fallback: Return allowed to prevent blocking users if Firestore connection fails temporarily
    return { allowed: true, currentCount: 1, limit: dailyLimit };
  }
}
 ...
 * Does NOT store raw PDF or sensitive document text.
 */

...
import { checkAndIncrementDailyUsageTransaction, writeAiUsageLog, getAdminFirestore } from './firestore';
...
export interface RateLimitEvent {
  ...
  endpoint: string;
  uid?: string;
  ipHash: string; // Privacy-safe SHA-256 hash prefix
  timestamp: number;
...
export interface SecurityEvent {
  id: string;
  type: 'auth_failure' | 'unauthorized_access' | 'rate_limit_block';
  uid?: string;
  endpoint: string;
  timestamp: number;
  details?: string;
...
/**
 * Gemini Pricing Configuration (USD per 1,000,000 tokens)
 * Config-driven with environment variable override fallback.
 */
export const PRICING_CONFIG = {
  // Gemini 2.5 Flash default pricing tier
...
/**
 * Server-side Telemetry & Observability Engine (Phase 14)
 * - In-memory ring buffers for high-frequency telemetry and rate-limit events.
 * - Firestore transactions for persistent daily atomic quotas and permanent audit records.
 * - Safe privacy hashing for client IP addresses.
...
class TelemetryStore {
  ...
  public alertThresholds = {
    ...
    avgLatencyMs: 1500, // Alert if avg latency > 1500ms
    highRateLimitPerHour: 20, // Alert if > 20 rate-limit blocks in 1 hour
  };
  ...
  /**
   * Helper to generate privacy-safe truncated hash of IP address
   */
  ...
  /**
   * Check if request is allowed under per-minute burst rate limit (10 req/min UID, 20 req/min IP)
   */
  public checkPerMinuteLimit(uid: string, ip: string, maxPerMin = 10): { allowed: boolean; retryAfterSec?: number } {
    ...
    // 2. Check IP per-minute limit (20 req/min per IP to prevent flood attacks)
  ...
  public async checkAndIncrementDailyQuota(
    ...
    return await checkAndIncrementDailyUsageTransaction(uid, dailyLimit);
  }
  ...
  /**
   * Log completed AI request execution details for persistent usage auditing and observability.
   */
  ...
  /**
   * Compute comprehensive system observability and cost analytics
   */
  public getSystemMetrics() {
    ...
    // Cost estimation using configuration rates
